...
namespace acstc::utils {
    ...
    auto ekc_callback(const size_t k, DCallback&& data_callback, CCallback&& count_callback) {
        return _impl::ekc_callback(k,
                                   _impl::propagate(data_callback, _impl::_empty<DCallback>{}),
                                   _impl::propagate(count_callback, _impl::_empty<CCallback>{}));
    }
    ...
    auto progress_callback(const size_t k, DCallback&& data_callback, const size_t& level = 2) {
        return progress_callback(k, verbosity::instance(), std::forward<DCallback>(data_callback), level);
    }
    ...
    auto callbacks(Callbacks&&... callbacks) {
        return [callbacks=std::tuple<Callbacks...>(std::forward<Callbacks>(callbacks)...)]
            (auto&&... values) mutable {
            _impl::caller<sizeof...(Callbacks) - 1>::call(callbacks, std::forward<decltype(values)>(values)...);
        };
    }
